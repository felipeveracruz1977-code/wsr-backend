-- 088_dashboard_scope_to_coached_alumnas.sql
-- El tab "Dirección" (fn_dashboard_kpis / fn_dashboard_risk_scores, migración
-- 047) contaba como "corredora activa" a cualquier runner con
-- status IN ('activa','miembro','miembro_oficial'), sin filtrar por si tiene
-- coach asignado. Eso mezclaba comunidad general (1564 de 1570 runners en
-- ese universo) con alumnas reales bajo seguimiento de un coach (6) — y como
-- la comunidad nunca tuvo plan ni hizo check-in, el scoring de abandono les
-- daba 60pts "alto" por defecto (fn_dashboard_risk_scores, caso
-- lc.created_at IS NULL), inundando el Panel de Riesgo con gente que ningún
-- coach está siguiendo.
--
-- Alcance corregido: "alumna" para este panel = runners.coach_id IS NOT NULL
-- (decisión confirmada con el usuario 2026-08-12). Deja fuera a runners con
-- plan pero sin coach vinculado (ej. planes generados sin coach logueado) —
-- ese caso puede revisarse aparte si se pide.
...
COMMENT ON FUNCTION public.fn_dashboard_kpis() IS
  'Executive dashboard KPI strip: active runners (con coach asignado), 28-day adherence average, and pending health_alerts by severity. Invoker rights — relies on *_admin_all RLS policies.';
...
COMMENT ON FUNCTION public.fn_dashboard_risk_scores() IS
  'Executive dashboard abandonment-risk table: one row per coached alumna (runners.coach_id IS NOT NULL) with pre-computed score/level. Invoker rights — relies on *_admin_all RLS policies.';
